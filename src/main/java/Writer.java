...
public class Writer {
  ...
  public Writer(GraphDatabaseService graphdb, String dateiname, String btg,
                String Path) {
  ...
  private void writeDigraph(Map<Long, List<Long>> btgNodesMap)
    throws IOException {
    try (Transaction tx = graphDB.beginTx()) {
      for (Map.Entry<Long, List<Long>> btgWithNodes : btgNodesMap.entrySet()) {
        // digraph header
        fileWriter.write(String
          .format("%s %d %s", DOT_DIGRAPH_HEADER, btgWithNodes.getKey(),
            DOT_BLOCK_OPEN));
        fileWriter.newLine();
        // nodes
  ...
  private void writeNodes(List<Long> nodes)
    throws IOException {
  ...
  private void writeEdges(List<Long> nodes)
    throws IOException {
    for (Long nodeID : nodes) {
      Node currentNode = graphDB.getNodeById(nodeID);
      for (Relationship edge : currentNode
